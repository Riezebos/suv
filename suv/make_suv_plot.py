...
SELECT 
    `datum_eerste_toelating` AS datum,
    COUNT(*) AS aantal 
WHERE 
(`voertuigsoort` = 'Personenauto') 
AND ((`bruto_bpm` > '5000')) 
AND ((`massa_rijklaar` > '2000')) 
AND ((`aantal_zitplaatsen` > '3' AND `aantal_zitplaatsen` < '8') AND `aantal_zitplaatsen` IS NOT NULL) 
AND (`inrichting` = 'stationwagen' OR `inrichting` = 'MPV') 
AND ((`vervaldatum_apk` > '20200427')) 
AND ((`datum_eerste_toelating` > '20000101')) 
GROUP BY datum 
...
# %%
df.datum = pd.to_datetime(df.datum).dt.strftime("%Y-%m")
df.aantal = df.aantal.astype(int)
df_agg = df.groupby("datum").sum()
df_agg = df_agg.reset_index()

# %%
px.line(df_agg,x="datum",y='aantal')
